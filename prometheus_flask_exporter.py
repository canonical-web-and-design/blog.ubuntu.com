"""
A fork of https://github.com/rycus86/prometheus_flask_exporter/,
to add the group_by_endpoint option, so metrics are grouped
by the entrypoint function that processes them, rather than
by their URL - so we don't end up with so many metrics
"""
...
class PrometheusMetrics(object):
    ...
    def histogram(self, name, description, labels=None, **kwargs):
        ...
        :param labels: label dictionary of `{labelname: callable_or_value}`
    ...
    @staticmethod
    def _track(
        metric_type, metric_call, metric_kwargs, name, description,
        labels, registry, before=None
    ):
        """
        Internal method decorator logic.

        :param metric_type: the type of the metric from the
            `prometheus_client` library
        :param metric_call: the invocation to execute as
            a callable with `(metric, time)`
        :param metric_kwargs: additional keyword arguments for
            creating the metric
        :param name: the name of the metric
        :param description: the description of the metric
        :param labels: a dictionary of `{labelname: callable_or_value}` for
            labels
    ...
    def info(
        self, name, description, labelnames=None, labelvalues=None,
        **labels
    ):
        """
